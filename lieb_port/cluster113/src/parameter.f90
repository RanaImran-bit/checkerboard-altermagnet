...
integer, parameter :: NKPTS = lx*ly

integer::NSTATES,NHS
! LIEB: 3 sites per unit cell, and lx=ly=2*cells, so the real-site count
! is 3*(lx/2)*(ly/2) = 3*lxy/4, NOT lxy.
parameter (NSTATES=3*(lx/2)*(ly/2),NHS=NSTATES*NSTATES)
...
parameter (NUP=6,NDN=6,NELEC=NUP+NDN,NSPIN=2)
parameter (NWFBAS=1,NIW=1,NWLKRS=1000,M_MAX=125)
parameter (NAVE=ntypes*NSTATES*NSTATES+ntypes*NKPTS+6)
...
integer::icorx(NKPTS),icory(NKPTS),ipx(4),ipy(4),ippx(4),ippy(4),idx(4),idy(4),ncor(lx,ly) 
...
integer::lbpx,lbpy,jnb    ! Lieb sublattice parity and a guarded neighbour index
                          ! (ipx/ipy are taken: pairing form-factor offsets)

!for specify the k-point 1-kx 2-ky

real(sp)::kSet(NKPTS,2)
...
real(sp), target::nk_up(NKPTS),nk_dn(NKPTS),nk_w(NKPTS)
real(sp), target::cdw(NKPTS),sdwz(NKPTS),sdwx(NKPTS)
real(sp), target::pmdf(NKPTS),dsf(NKPTS)
...
real(sp), target::gxk_up(NKPTS),gxk_dn(NKPTS)
...
real(sp), target :: swave(NKPTS), dwave(NKPTS), pwave(NKPTS), sowave(NKPTS)
real(sp), target :: sbwave(NKPTS), dbwave(NKPTS), pbwave(NKPTS)
real(sp), target :: puupxwave(NKPTS), pddpywave(NKPTS), puupywave(NKPTS), pddpxwave(NKPTS)
real(sp), target :: pudpxwave(NKPTS), pudpywave(NKPTS)
real(sp), target :: pdsfbd1(NKPTS), pdsfbd2(NKPTS), pdsfbd12(NKPTS)
real(sp), target :: dd1wave(NKPTS), dd2wave(NKPTS), dd12wave(NKPTS)
real(sp), target :: pbdx2y2d1(NKPTS), pbdx2y2d2(NKPTS), pbdx2y2d12(NKPTS)
real(sp):: kValsArr(ntypes * NKPTS)
