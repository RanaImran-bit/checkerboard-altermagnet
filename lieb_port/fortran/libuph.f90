...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
      ...
      do jbas=1,NIW
         do ibas=1,NWFBAS
            call dgemm('N', 'N', NUP, NUP, NSTATES, 1.0_8, phiZ_up(:,:,ibas), NUP, phiB_up(:,:,jbas), NSTATES, 0.0_8, temp_up, NUP) !li
             
            !temp_up=matmul(phiZ_up(:,:,ibas),phiB_up(:,:,jbas))
            !write(*,*)temp_up

            call sgefa(temp_up,NUP,NUP,ipvt,info)
	    if(info /= 0) stop 'Problem in sgefa routine3'
            call sgedi(temp_up,NUP,NUP,ipvt,det,work,10_i4b)
	    det_up(ibas)=det(1)*TEN**det(2)
            
            call dgemm('N', 'N', NDN, NDN, NSTATES, 1.0_8, phiZ_dn(:,:,ibas), NDN, phiB_dn(:,:,jbas), NSTATES, 0.0_8, temp_dn, NDN) !li

            !temp_dn=matmul(phiZ_dn(:,:,ibas),phiB_dn(:,:,jbas))

            call sgefa(temp_dn,NDN,NDN,ipvt,info)
	    if(info /= 0) stop 'Problem in sgefa routine4'
            call sgedi(temp_dn,NDN,NDN,ipvt,det,work,10_i4b)
...
600   continue
      ...
      write(*,*)'Initial Energy Per N=',etrial/real(nsites)
      write(*,*)'###########################################'
      write(*,*)
      ...
      write(*,*)'Growth estimate of Etrial Per N: ',etrial/real(nsites)
      write(*,*)
      ...
      do 600 iw=1,NWLKRS
        ...
        do 700 ibas=1,NWFBAS
         ...
         if(info /= 0) stop 'Problem in sgefa routine5'
         ...
         if(info /= 0) stop 'Problem in sgefa routine6'
...
100    continue
      ...
      do ibas=1,NWFBAS
         ...
         call sgefa(ovlpINV_up,NUP,NUP,ipvt,info)
         if(info /= 0) stop 'Problem in sgefa routine7'
         call sgedi(ovlpINV_up,NUP,NUP,ipvt,det,work,11_i4b)
         det_up(ibas)=det(1)*TEN**det(2)
       ...
       !! Rename G to be the inverse,
         ...
         call sgefa(ovlpINV_dn,NDN,NDN,ipvt,info)
         if(info /= 0) stop 'Problem in sgefa routine8'
         call sgedi(ovlpINV_dn,NDN,NDN,ipvt,det,work,11_i4b)
         det_dn(ibas)=det(1)*TEN**det(2)
