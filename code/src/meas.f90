...
! inter-orbital on-site U' (uxy): uxy * <n_r n_{r+lxy}> over real sites r=1..lxy
! <n_a n_b> = (gxu_aa+gxd_aa)(gxu_bb+gxd_bb) - gxu_ab gxu_ba - gxd_ab gxd_ba  (a/=b)
if(abs(uxy)>0.001) then
    do i=1,lxy
        j=i+lxy
        hpot=hpot+uxy*( (gx_up(i,i)+gx_dn(i,i))*(gx_up(j,j)+gx_dn(j,j)) &
                       - gx_up(i,j)*gx_up(j,i) - gx_dn(i,j)*gx_dn(j,i) )
    end do
end if
! inter-orbital nearest-neighbour V (sign -v vs +v intra above): on +x/+y bonds,
! couple orbital 1 at site to orbital 2 at neighbour and vice versa.
if(abs(v)>0.001) then
    do i=1,lxy
        do j1=1,2
            j=idis(i,j1)            ! orbital-1 neighbour real position; +lxy -> orbital 2
            hpot=hpot - v*( (gx_up(i,i)+gx_dn(i,i))*(gx_up(j+lxy,j+lxy)+gx_dn(j+lxy,j+lxy)) &
                           - gx_up(i,j+lxy)*gx_up(j+lxy,i) - gx_dn(i,j+lxy)*gx_dn(j+lxy,i) ) &
                       - v*( (gx_up(i+lxy,i+lxy)+gx_dn(i+lxy,i+lxy))*(gx_up(j,j)+gx_dn(j,j)) &
                           - gx_up(i+lxy,j)*gx_up(j,i+lxy) - gx_dn(i+lxy,j)*gx_dn(j,i+lxy) )
        end do
    end do
end if
have=hkin+hpot
