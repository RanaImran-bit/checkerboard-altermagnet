...
echo "$(date '+%F %T') predecessor finished, starting magnetic" >> $HOME/chain_255.log
...
SIZES=10 US=0,2,4 DELTAS=0.5,0.6,0.7 NPROC=32 NSEED=6 \
  /opt/anaconda3/bin/python checkerboard_mag_scan.py < /dev/null > mag_hi2.log 2>&1
echo "$(date '+%F %T') magnetic done, starting pairing" >> $HOME/chain_255.log

cd $HOME/pair_hi2
SIZES=10 US=0,2,4 DELTAS=0.5,0.6,0.7 NPROC=32 NSEED=6 \
  /opt/anaconda3/bin/python checkerboard_fss_full.py < /dev/null > pair_hi2.log 2>&1
echo "$(date '+%F %T') pairing done, node idle" >> $HOME/chain_255.log
