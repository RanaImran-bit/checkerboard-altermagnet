...
already for free electrons. Figure~\ref{fig:eqvertex} contrasts the two.
...
VERSIONS = {
 "v1_pairing_and_magnetism.tex": (
   ...
   [INTRO_COMMON, V1_INTRO_TAIL, MODEL, ORDER_PARAM, MAGNETISM, NULL, FSS,
 ...
 "v3_magnetism_only.tex": (
   ...
   V3_ABS,
   [INTRO_COMMON, V3_INTRO_TAIL, MODEL, ORDER_PARAM, MAGNETISM, NULL, FSS,
    ROBUST, V3_CONC, APPENDIX, BIB]),
}

for fname, (title, abstract, blocks) in VERSIONS.items():
    body = preamble(title, abstract) + "".join(blocks)
    p = os.path.join(OUT, fname)
    open(p, "w").write(body)
    figs = body.count("includegraphics")
    print(f"{fname}: {len(body.splitlines())} lines, {figs} figures")
