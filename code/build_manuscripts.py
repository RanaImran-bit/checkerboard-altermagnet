...
"""Assemble the three manuscript versions from shared text blocks.

  v1  pairing and magnetism together (the full story)
  v2  pairing only
  v3  magnetism only

Written as one generator rather than three files so the model, method, and
appendix text cannot drift apart between versions. Every number carries a comment
naming the file it comes from, matching the convention the original draft set.
House style: no em-dashes, no semicolons in prose, d_xy and d_x2-y2 rather than
the B2g and B1g labels.
"""
import os, textwrap

OUT = os.path.expanduser("~/Desktop/checkerboard-altermagnet/manuscript/versions")
os.makedirs(OUT, exist_ok=True)

# ---------------------------------------------------------------- preamble --
def preamble(title, abstract):
    return r"""%% ============================================================================
%%  Physical Review B
%%  """ + " ".join(title.replace("$", "").replace("\\", "").split()) + r"""
%%
...
# ------------------------------------------------------------------ builds --
V1_ABS = r"""We study the half-filled Hubbard model on the checkerboard lattice by
...
V2_ABS = r"""We study pairing in the half-filled Hubbard model on the checkerboard
...
V3_ABS = r"""We study magnetic correlations in the half-filled Hubbard model on the
...
ROBUST   = ROBUST_TEMPLATE.replace('@@BCPARA@@', r'''Appendix~\ref{app:bc} reports a repeat with antiperiodic boundaries, which shift
...
ROBUST_V2 = ROBUST_TEMPLATE.replace('@@BCPARA@@', r'''Appendix~\ref{app:bc} reports a repeat with antiperiodic boundaries, which shift
...
VERSIONS = {
 "v1_pairing_and_magnetism.tex": (
   r"Emergent altermagnetic correlations and $d_{xy}$ pairing in the" "\n"
   r"checkerboard Hubbard model", V1_ABS,
   [INTRO_COMMON, V1_INTRO_TAIL, MODEL, ORDER_PARAM, MAGNETISM, MECHANISM, NULL, FSS,
    PAIRING, LINK, ROBUST, V1_CONC, APPENDIX, BIB]),
 "v2_pairing_only.tex": (
   r"Interaction-induced $d_{xy}$ pairing in the checkerboard Hubbard model",
   V2_ABS,
   [INTRO_COMMON, V2_INTRO_TAIL, MODEL, PAIRING, ROBUST_V2, V2_CONC, APPENDIX, BIB]),
 "v3_magnetism_only.tex": (
   r"Emergent altermagnetic correlations in the checkerboard Hubbard model",
   V3_ABS,
   [INTRO_COMMON, V3_INTRO_TAIL, MODEL, ORDER_PARAM, MAGNETISM, MECHANISM, NULL, FSS,
    ROBUST, V3_CONC, APPENDIX, BIB]),
}

def prune_bibliography(body):
    """Drop \\bibitem entries this version never cites.

    The three versions share one reference pool, but the magnetism-only paper has
    no pairing section and so does not cite the square-lattice superconductivity
    literature. Carrying those entries would leave a reference list with items no
    sentence points at, which a referee reads as padding."""
    import re
    cited = set()
    for c in re.findall(r"\\cite\{([^}]*)\}", body):
        cited |= {k.strip() for k in c.split(",")}
    def keep(m):
        return m.group(0) if m.group(1) in cited else ""
    return re.sub(r"\\bibitem\{([^}]*)\}\n[^\n]*\n[^\n]*\n\n", keep, body)

for fname, (title, abstract, blocks) in VERSIONS.items():
    body = prune_bibliography(preamble(title, abstract) + "".join(blocks))
    p = os.path.join(OUT, fname)
    open(p, "w").write(body)
    figs = body.count("includegraphics")
    nref = body.count("\\bibitem")
    print(f"{fname}: {len(body.splitlines())} lines, {figs} figures, {nref} references")
