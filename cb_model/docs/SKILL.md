...
## Layer 7: Physics manuscripts (PRB/PRL/PRX and condensed-matter)
Register: dense, equation-driven, results carried by figures; third-person plural "we"; neutral and
precise. The abstract is a single paragraph with no citations and no undefined acronyms; it names the
model, the method, and the main result with its key number or symmetry.

- **Define every symbol at first use** and keep notation consistent; never cycle names or notation for one
  quantity (the elegant-variation ban, Layer 1, applies doubly to symbols).
- **Punctuate displayed equations** as part of the sentence (a comma or period after the equation).
- **Abbreviate numbered cross-references**: "Fig. 1", "Eq. (3)", "Table I", "Ref. [12]" (capitalized when
  numbered; lowercase generic, "the figure shows"). Use `\cref`/`\eqref` consistently.
- **State the energy scale/units once** ("energies in units of the hopping t") and keep it.
- **Do not begin a sentence with a bare symbol or numeral**; recast.
- **Match sub-field conventions from the cited papers** (sign of the hopping, order-parameter definition,
  Brillouin-zone and Fourier conventions); note a convention rather than silently changing it, and never
  alter a borrowed equation's meaning.
- **Method claims match the method's guarantees.** Constrained-path / auxiliary-field QMC carries a
  controlled bias: write "within the constrained-path approximation" and report the benchmark against
  exact diagonalization, not "exact" or "numerically exact" unless the method is sign-problem-free.
  Mean-field or schematic figures are labeled as such, with illustrative parameters flagged (Layer 4).
- **Order-parameter care.** State the order parameter and its symmetry explicitly (e.g., $d_{xy}$ vs
  $d_{x^2-y^2}$); call a state "compensated" or "zero net moment" only where that is shown, not asserted.
